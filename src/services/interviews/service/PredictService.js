...
class PredictService {
  ...
  async predictEmotion(file) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) throw ApiError.internal('Emotion API key tidak valid');
      if (response.status === 400) throw ApiError.badRequest(`File tidak valid: ${text || 'bad request'}`);
      throw new ApiError(response.status, `Layanan emotion detection error: ${text || response.statusText}`);
    }

    return this._enrichResult(await response.json());
  }

  async healthCheck() {
    try {
      const res = await fetch(`${this.baseUrl}/health`);
      if (!res.ok) throw new ApiError(res.status, 'Health check gagal');
  ...
  _enrichResult(data) {
    ...
    return {
      ...
      confidence,
      confidencePercent: confidence !== null ? Number((confidence * 100).toFixed(2)) : null,
